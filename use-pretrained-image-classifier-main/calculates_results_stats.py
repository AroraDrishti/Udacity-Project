...
#          Please see "Intro to Python - Project classifying Images - xx Calculating Results"
#          for details on how to calculate the counts and percentages for this function.
...
def calculates_results_stats(results_dict):
    ...
    Parameters:
      results_dict - Dictionary with key as the image filename and value as a List 
    ...
    results_stats_dict['n_images'] = len(results_dict.items())

    for key, pet_labels in results_dict.items():
        ...
        if results_dict[key][3] == 0:
            results_stats_dict['n_notdogs_img'] += 1

        if results_dict[key][3] == 0 and results_dict[key][4] == 0:
    ...
    if results_stats_dict['n_dogs_img'] > 0:
        results_stats_dict['pct_correct_dogs'] = (
            results_stats_dict['n_correct_dogs'] / results_stats_dict['n_dogs_img']) * 100
    if results_stats_dict['n_notdogs_img'] > 0:
        results_stats_dict['pct_correct_notdogs'] = results_stats_dict['n_correct_notdogs'] / \
            results_stats_dict['n_notdogs_img'] * 100
    if results_stats_dict['n_dogs_img'] > 0:
        results_stats_dict['pct_correct_breed'] = results_stats_dict['n_correct_breed'] / \
            results_stats_dict['n_dogs_img'] * 100
    if results_stats_dict['n_images'] > 0:
        results_stats_dict['pct_match'] = results_stats_dict['n_match'] / \
            results_stats_dict['n_images'] * 100
